...
export class PeriodsService {
  ...
  splitDates = [
    {
      id: 'car',
      splitdates: () => {
        // is fn to make it ready for dynamic types
  ...
  getPeriodOptions(type: string): periodOptions[] {
    ...
    const currentDate = new Date(),
      ...
      uniqueSplits = Object.values(
        splits.reduce((store: Object, val: Date) => {
          if (!store[val.getFullYear()]) {
            // the year is new / unique
            store[val.getFullYear()] = new CreatePeriod(null, null, [val]);
          } else {
            // push to an existing year
